...
- **Certificate rearranger migrated to ESM/TypeScript/React** (#921). The interactive element repositioning UI on `rearrange.php` now mounts a React component via Moodle's `data-react-component` auto-init (`@moodle/lms/mod_customcert/Rearrange`). Drag positioning, keyboard nudging, and click-to-edit live in `js/esm/src`. Save and close / Save and continue / Cancel controls are owned by the React rearranger (no PHP `single_button` markup or DOM query wiring). After editing an element, generic layout properties (`posx`, `posy`, `width`, `refpoint`, `alignment`, preview `html`) returned by `mod_customcert_save_element` are merged into React state immediately so the interactive editor stays in sync without a page reload. Element preview HTML and edit forms remain server-rendered (`render_html()` and the `editelement` fragment) so third-party `customcertelement_*` plugins continue to work without frontend changes. The legacy YUI module `moodle-mod_customcert-rearrange` and AMD modules `mod_customcert/rearrange-area` and `mod_customcert/dialogue` have been removed.
