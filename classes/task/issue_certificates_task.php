...
class issue_certificates_task extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        if (empty($customcerts)) {
            if ($this->VERBOSE) {
                mtrace("No certificates to be processed, terminating.");
            }
            return;
        }
        ...
        if ($this->VERBOSE) {
            ...
            mtrace("Found $n_certs certificates to be processed");
        }

        foreach ($customcerts as $customcert) {
            mtrace("Processing certificate ID#$customcert->id - Name: $customcert->name - Course ID#$customcert->courseid");
            ...
            // (Note: filter_user_list only takes into account those availability condition which actually implement this function, so the second check with get_fast_modinfo must be still performed - but we can reduce the size of the users list here already)
            $infomodule = new \core_availability\info_module($cm);
            $filteredusers = $infomodule->filter_user_list($userswithissueview);

            if ($this->VERBOSE) {
                $n_users = count($filteredusers);
                mtrace("> Found $n_users users to be processed");
            }
            ...
            if ($this->VERBOSE) {
                mtrace("> Issued $new_certs new certificates");
            }
        }
    }
}
