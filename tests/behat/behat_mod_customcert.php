...
class behat_mod_customcert extends behat_base {
    ...
    /**
     * Directs the current user to the report-download URL for another user's certificate PDF.
     *
     * Used to test that the report-download flow verifies an issue exists for the target user
     * before generating a PDF, rather than trusting the downloadissue request parameter.
     *
     * @Given /^I attempt to download the certificate report PDF for "(?P<user_name>(?:[^"]|\\")*)" in "(?P<certificate_name>(?:[^"]|\\")*)"$/
     * @param string $username
     * @param string $certificatename
     */
    public function i_attempt_to_download_the_certificate_report_pdf_for($username, $certificatename) {
        global $DB;
        ...
        $url = new moodle_url('/mod/customcert/view.php', ['id' => $cm->id, 'downloadissue' => $user->id]);
        $this->getSession()->visit($this->locate_path($url->out_as_local_url()));
    }
